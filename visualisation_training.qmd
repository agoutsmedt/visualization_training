...
**=>Plots are built incrementally by adding the components piece by piece**
:::
...
data_ispole%>%
  ...
  theme_pubclean()+
  theme(plot.title =element_text(size=20,hjust=0.5))+  labs(x=NULL,y=NULL,title="Evolution of ISPOLE's Publication Activity Over Time" )
``` 
...
#| fig-height: 3.5
#| fig-width: 8
data_ispole%>%select(type_de_publication,n_titre)%>%glimpse
``` 
